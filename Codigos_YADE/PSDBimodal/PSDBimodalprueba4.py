#**********************************************************************
#GRAN FRACASO, MAL ENFOQUE, UN DESPLAZAMIENTO PARA CADA OBJETO.
#CREO QUE EL ANTERIOR ESTA BUENO, TE ABANDONO GUAPO

#Cuadrilatero con agua, PSD bimodal
#acoplado con flujo (ok)
#eliminado funciones kfin y kini, aumento de SD (ok)
#forma 2 de sacar el desplazamiento
...
#disminuir Young mejora delta(t ) aumentar densidad tb
#******************************************************************
...
for i in range(0, n1):
    volume=Rad[i]**3/Voltot
    result=repr(Rad[i])+" "+repr(volume)+"\n"
...
def before():
    ...
    cant3=0
    actual=O.iter
    inicial=despb.firstIterRun
    for i in range(6, n1+6):
        rii=O.bodies[i].shape.radius
        if actual<=inicial:
         PosBefore=O.bodies[i].state.refPos
        else:
         PosBefore1=__builtin__.PosNow2
        PosNow1=O.bodies[i].state.se3[0]
        deltaVect1=PosNow1-PosBefore1
        deltaScalar1=math.sqrt(numpy.dot(deltaVect1, deltaVect1))
        if 2*rii<D15:
         despFino1[cant2]=deltaScalar
         d2=d2+deltaScalar
         cant2=cant2+1
        if 2*rii>D85:
         despGros1[cant1]=deltaScalar
         d1=d1+deltaScalar
         cant1=cant1+1
        else: 
#         despRest[cant3]=deltaScalar
         cant3=cant3+1
    if actual<=inicial:
     __builtin__.despAcumFino1=despFino1
    else:     
     __builtin__.despAcumFino1=despFino1+__builtin__.despAcumFino2
    despMaxFino1=numpy.amax(despFino1)
    despAcumMaxFino1=numpy.amax(__builtin__.despAcumFino1)
    despMeanFino1=d2/cant2
    if inicial<=graf.firstIterRun:
     __builtin__.despAcumGros1=despGros1
    else:     
    __builtin__.despAcumGros1=despGros1+__builtin__.despAcumGros2
    despMaxGros1=numpy.amax(despGros1)
    despAcumMaxGros1=numpy.amax(__builtin__.despAcumGros1)
    despMeanGros1=d1/cant1
    O.engines[4].previousDt
...
#         despRest[cant3]=deltaScalar
    ...
    if inicial==graf.firstIterRun:
     ...
     __builtin__.despAcumGros=__builtin__.despAcumGros+despGros
    __builtin__.despMaxGros=numpy.amax(despGros)
    __builtin__.despAcumMaxGros=numpy.amax(__builtin__.despAcumGros)
    __builtin__.despMeanGros=d1/(cant1*1.)
    gross=a1*1.
    fino=a2*1.
    resto=a3*1.
    ...
    plot.addData(dmf=despMeanFino, dMaxf=despMaxFino,dMaxAcumf=despAcumMaxFino, i=O.iter, i1=O.iter, i2=O.iter, ic=__builtin__.icr, ii=it)
...
#    print "AcumFinoB",despAcumFinoMaxB
...
PyRunner( command='aumentapresure()', iterPeriod=8000, firstIterRun=100000, label="presion"),
newton,
PyRunner(command='parar()', iterPeriod=50, firstIterRun=81100, label="muro"),
PyRunner(command='ploteo()', iterPeriod=100, label='graf',firstIterRun=100),
PyRunner(command='possBefore()', iterPeriod=100, label='auxiliar',firstIterRun=50),
]

#plot.plots={'i':('Gf'), 'i1':('Gg')}
plot.plots={'i':('dMaxf', 'dmf', 'dMaxAcumf'), 'i2':('ic', 'ii') }
plot.plot()
O.run()
