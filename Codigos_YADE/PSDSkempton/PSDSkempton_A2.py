...
####################################################################
###############  Agregado a granulometria B   ########################
####################################################################
mn2=Vector3(0,0,0.36)
mx2=Vector3(D,D,0.86)
sph2=pack.SpherePack()
sph2.makeCloud(mn2,mx2,-1,0,num_spheres,False, 0.65,psdSizes,psdCumm,True,seed=1)
...
for i in range(0, n1):
    ...
    file = open("/home/diego/memoria/YADE/Codigos/PSDSkempton/granSkempton_DX1", "a")
    file.write(result)
    file.close()
...
def ploteo():
    ...
    print "time= ", __builtin__.time1
    print "Gf=",Gfp," Gg",Ggp,"Tf=",tensfp,"Tg=",tensgp, "Tmf=", Tmf 
    cant1=0
    cant2=0
    cant3=0
...
PyRunner(command='perfilVertTens', iterPeriod=1000, firstIterRun=2000, label="perfilV", dead=0),
