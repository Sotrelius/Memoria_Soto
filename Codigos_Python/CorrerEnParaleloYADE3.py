...
#la wea le meto n fijo rmean fijo a distintos sd, curvas biliniales
####################################################
...
def each_call(input_params):
    ...
    name, process, Vel, Sd, np, Pi = input_params #le agregue mis putos parametros madafaka
    Repnp=repr(np)
    RepSd=repr(Sd)
    archivo='/home/diego/memoria/YADE/Codigos/RunsN/PsdVarPRFX'+repr(process)
    file = open(archivo, 'a')
...
"O.bodies[5].state.vel[2]=-"+repr(Vel)+" # era -0.05, pero agilizamos la wa\n"
"\n"
"k1=0    \n"
"k2=0\n"
"n=1\n"
...
"def parar():\n"
"   poros=flow.porosity\n"
"   O.bodies[5].state.vel[2]=-"+repr(Vel*0.1)+"#agilizando la cosa\n" #disminuir
"   O.pause()\n"
"   flow.dead=0\n"
...
"   conf=O.forces.f(5)[2]/D/D\n"
"   if abs(err)<1e-3:\n"
"    O.pause()\n" 
"    file = open('/home/diego/memoria/Codigos/analisisBL/resulconfBL', 'a')\n"
...
"\n")
    file.close()
    ruta='/home/diego/memoria/YADE/Codigos/RunsN/'
    fileName='PsdVarPRF'+RepSd[2:]+'R'+Repnp[2:]+'.py'
    command_name = 'touch '+ruta+fileName+' && cd '+ruta+' && cat '+archivo+' > '+fileName+' && cat /dev/null > '+archivo+ ' && cd && yade-1.14.0 /home/diego/memoria/YADE/Codigos/RunsN/'+fileName   # Change for whatever commnad you need to call
...
inputs = [ ('Sotrero', 1, Vel, 0.34, np, Pi)]#, ('Hamilton', 2, Vel, 0.17, np, Pi), ('verstapen', 3, Vel, 0.34, np, Pi), ('Skt1MPhelps', 4, Vel, 0.51, np, Pi ), ('UsainBolt', 5, Vel, 0.68, np,Pi)]
